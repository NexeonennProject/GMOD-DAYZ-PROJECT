translate.AddLanguage("ru", "Russian")

--Language label
LANGUAGE.language_name = "Русский"

--AFK
LANGUAGE.AFKText1 = "Вас не было около "
LANGUAGE.AFKText2 = "ОТОШЁЛ"

--GRAVE
LANGUAGE.Grave = "Могила"
LANGUAGE.GraveText1 = "Умер по неизвестной причине"
LANGUAGE.GraveText2 = "Нажмите 'E' чтобы отдать честь"

--AUTORESTART
LANGUAGE.AutoRestart1 = "ВНИМАНИЕ"
LANGUAGE.AutoRestart2 = "Через %s секунд будет произведён рестарт сервера!"
LANGUAGE.AutoRestart3 = "Во избежание потери лута, встаньте где-нибудь в безопасном месте"

--Other
LANGUAGE.money = "ДЕНЬГИ"
LANGUAGE.backpack = "РЮКЗАК"
LANGUAGE.item = "ПРЕДМЕТ"
LANGUAGE.neutral = "НЕЙТРАЛ"
LANGUAGE.bandit = "БАНДИТ"
LANGUAGE.hero = "ГЕРОЙ"

LANGUAGE.StringGoodTime1 = "несколько секунд"
LANGUAGE.StringGoodTime2 = " секунд(ы)"
LANGUAGE.StringGoodTime3 = " минут(а,ы)"
LANGUAGE.StringGoodTime4 =  " час(ов)"
LANGUAGE.StringGoodTime5 = " день"
LANGUAGE.StringGoodTime6 = " неделя"
LANGUAGE.StringGoodTime7 = " год"

--Scoreboard
LANGUAGE.scoreboard_ping = "Пинг"
LANGUAGE.scoreboard_kills = "Убийства"
LANGUAGE.scoreboard_lvl = "Уровень"
LANGUAGE.scoreboard_status = "Ранг/Префикс"
LANGUAGE.scoreboard_stats = "        Статус"
LANGUAGE.scoreboard_group = "Группа"

LANGUAGE.scoreboard_ButtonSteamID = "Скопировать SteamID"
LANGUAGE.scoreboard_ButtonCopyName = "Скопировать никнейм"
LANGUAGE.scoreboard_ButtonShow = "Посмотреть профиль"

LANGUAGE.scoreboard_ButtonGroupKick = "Выгнать из группы"
LANGUAGE.scoreboard_ButtonGroupInvite = "Пригласить в группу"
LANGUAGE.scoreboard_ButtonGroupAcceptInvite = "Принять приглашение"
LANGUAGE.scoreboard_ButtonGroupLeave = "Выйти из группы"

LANGUAGE.scoreboard_alive = "Жив"
LANGUAGE.scoreboard_notalive = "Мертв"

LANGUAGE.scoreboard_muteplayer = "Заглушить/Разглушить игрока"

LANGUAGE.scoreboard_handsup = ""
--Animation
LANGUAGE.handsup = "Показать анимацию:Поднять Руки"
LANGUAGE.halt = "Показать анимацию:Остановись"
LANGUAGE.pointfingers = "Показать анимацию:Указывание пальцем"
LANGUAGE.cool = "Показать анимацию:Класс"
LANGUAGE.hi = "Показать анимацию:Приветствие"
LANGUAGE.followme = "Показать анимацию:За мной"
LANGUAGE.stopthat = "Показать анимацию:Нет"
LANGUAGE.sit = "Показать анимацию:Сесть"

--Message
LANGUAGE.killmessage = "Ты убил "

LANGUAGE.hungermessage = "Мне нужно найти что-нибудь поесть, иначе я умру."
LANGUAGE.thirstmessage = "Мне нужно найти что-нибудь попить, иначе я умру."

--pickup
LANGUAGE.pickup_item = "Нажми 'E' чтобы поднять"
LANGUAGE.loot_item = "Нажми 'E' чтобы залутать"

--weapon
LANGUAGE.gmod_camera = "Камера"
LANGUAGE.gmod_tool = "ТулГан"
LANGUAGE.weapon_physcannon = "ГравиПушка"
LANGUAGE.weapon_crowbar_dayz = "Монтировка"
LANGUAGE.weapon_bat_dayz = "Бита"
LANGUAGE.weapon_hl2axe = "Топор"
LANGUAGE.weapon_binoculars = "Бинокль"

--HUD
LANGUAGE.hint_tipUse = ""
LANGUAGE.hint_XpAmmount = "XP Получено"
LANGUAGE.hint_MoneyAmmount = " Подобрал"
LANGUAGE.hint_tipHelp = ""
LANGUAGE.hint_tipAdmin = "[АДМИН]"
LANGUAGE.hint_tipCanselUse = "[ОТМЕНА ДЕЙСТВИЯ]:"

LANGUAGE.hud_missingcontent1 = "НА ВАШЕМ КЛИЕНТЕ НЕХВАТАЕТ КОНТЕНТА!"
LANGUAGE.hud_missingcontent2 = "КОНТЕНТ: 'CSS' не найден!"
LANGUAGE.hud_missingcontent3 = "КОНТЕНТ: 'HL2:Episode 2' не найден!"
LANGUAGE.hud_missingcontent4 = "КОНТЕНТ: 'GMod DayZ' не найден!"
LANGUAGE.hud_missingcontent5 = "НАСЛАЖДАЙТЕСЬ ОШИБКАМИ/ЭМО ТЕКСТУРАМИ"
LANGUAGE.hud_missingcontent6 = "Нет,серьезно. Скачать контент можно в группе vk.com/official_russian_dayz"

LANGUAGE.radar_nickme = "Я"

LANGUAGE.deathLMBMsg = "Нажми ЛКМ чтобы зареспауниться."
LANGUAGE.deathLMBMsgCountDown = " секунд до респавна!"

LANGUAGE.GroupDistance = "Растояние:"

LANGUAGE.Vip = "Эта зона только для VIP'ов и выше!"

LANGUAGE.Tag = "Вы атаковали/атакованы!!! Вы сможете покинуть сервер/зайти на безопасную зону через %s секунд"
LANGUAGE.TagLabel = "ВЫ АТАКОВАНЫ"
LANGUAGE.TagText1 = ""
LANGUAGE.TagText2 = "Здесь вы БУДЕТЕ получать урон от других!"
LANGUAGE.TagText3 = "Доступ к магазину и банку закрыт!"
LANGUAGE.TagText4 = "Вы будете защищены через %s секунд(ы)."

LANGUAGE.SafeZoneLabel = "БЕЗОПАСНАЯ ЗОНА"
LANGUAGE.SafeZoneText1 = "Здесь вы не сможете использовать оружие!"
LANGUAGE.SafeZoneText2 = "Здесь вы можете купить оружие/сложить ненужные предметы в банк"
LANGUAGE.SafeZoneText3 = "Здесь вы не получите урон от других игроков."
LANGUAGE.SafeZoneText4 = ""

LANGUAGE.SafeZoneEdgeLabel = "КРАЙ БЕЗОПАСНОЙ ЗОНЫ"
LANGUAGE.SafeZoneEdgeText1 = ""
LANGUAGE.SafeZoneEdgeText2 = ""
LANGUAGE.SafeZoneEdgeText3 = "Здесь вы не будете получать урон от игроков!"
LANGUAGE.SafeZoneEdgeText4 = "Войдите в безопасную зону, чтобы получить доступ к банку/магазину."

LANGUAGE.VehicleHP = "Жизни: "
LANGUAGE.VehicleFuel = "Топливо: "

LANGUAGE.UseBank = "Нажми 'E' чтобы войти в Банк" 
LANGUAGE.UseShop = "Нажми 'E' чтобы войти в Магазин"
LANGUAGE.UseVipShop = "Нажми 'E' чтобы войти в [ВИП]Магазин"

--Chat
LANGUAGE.Global = "[Глобальный]"
LANGUAGE.Local = "[Локальный]"
LANGUAGE.Group = "[Группа]"
LANGUAGE.Advert = "[Объявление]"

--CreateCharacter
LANGUAGE.SelectGender = "Выберите пол"
LANGUAGE.SelectMan = "Парень"
LANGUAGE.SelectWoman = "Девушка"
LANGUAGE.SelectCharacter = "Выберите вашего персонажа"
LANGUAGE.SelectCharacter1 = "Тип тела"
LANGUAGE.SelectCharacter2 = "Одежда"
LANGUAGE.ConfirmCharacter = "Окей"
LANGUAGE.BackCharacter = "Назад"

--Inventory
LANGUAGE.YourBackPack = "Твой Рюкзак"
LANGUAGE.LootBackPack = "Лут"
LANGUAGE.CrateBackPack = "Ящик"
LANGUAGE.YourBankBackPack = "Твой Банк"
LANGUAGE.CloseBank = "Закрыть банк на Q: "
LANGUAGE.CloseInventory = "'Q' Закрыть инвентарь"

LANGUAGE.ItemName = "Название: "
LANGUAGE.ItemDesc = "Описание: "
LANGUAGE.ItemWeight = "Вес в слотах: "
LANGUAGE.PlaceItem = "Поставить предмет"
LANGUAGE.DropItem = "Выбросить предмет"
LANGUAGE.CooldownDropItem = "Вы не сможете выбросить предмет т.к вы уже используете какой-то предмет."
LANGUAGE.GuitarBlockDropItem = "Нельзя выбросить предмет,пока в руках гитара."
LANGUAGE.UseItem = "Использовать предмет"
LANGUAGE.CooldownUseItem = "Вы не сможете использовать предмет т.к вы уже используете какой-то предмет."
LANGUAGE.GuitarBlockUseItem = "Нельзя использовать предмет,пока в руках гитара."
LANGUAGE.CookItem = "Готовить предмет"
LANGUAGE.CraftCampfire = "Сделать костёр"
LANGUAGE.DestroyItem = "Уничтожить предмет"
LANGUAGE.DepositItem = "Положить предмет"
LANGUAGE.WithdrawItem = "Взять предмет"
LANGUAGE.AmountPopupItem = "Количество предметов"

LANGUAGE.Weight = "Слот(ов): "

LANGUAGE.BankFullText1 = "Банк полон, купи ВИП/GOLD чтобы у тебя было больше места!!!"
LANGUAGE.BankFullText2 = "Банк полон, купи GOLD чтобы у тебя было больше места!!!"
LANGUAGE.BankFullText3 = "Банк полон,больше его никак не увеличить!!!"

--TableSheet
LANGUAGE.InventaryTable = "Инвентарь"
LANGUAGE.InventaryTableDesc = "Здесь ваш инвентарь."
LANGUAGE.SkillTable = "Таланты/Перки"
LANGUAGE.SkillTableDesc = "Здесь все ваши таланты и перки."
LANGUAGE.CraftingTable = "Крафтинг"
LANGUAGE.CraftingTableDesc = ""
LANGUAGE.SettingsTable = "Настройки"
LANGUAGE.SettingsTableDesc = "Здесь все ваши настройки."
LANGUAGE.RulesTable = "Правила"
LANGUAGE.RulesTableDesc = "Здесь правила сервера."
LANGUAGE.SkillsTable = "Таланты"
LANGUAGE.SkillsTableDesc = "Все о талантах."
LANGUAGE.PerkTable = "Перки"
LANGUAGE.PerkTableDesc = "Все о перках."

--Settings
LANGUAGE.EnableContentText = "Включить сообщение о недостающем контенте:"
LANGUAGE.EnableHudText = "Включить Худ:"
LANGUAGE.NVButtonText = "Кнопка ПНВ:"
LANGUAGE.LieDownButtonText = "Кнопка Лечь:"
LANGUAGE.InvButtonText = "Кнопка Инвентаря:"
LANGUAGE.MapButtonText = "Кнопка Карты:"
LANGUAGE.EnableMinimapText = "Включить Миникарту:"
LANGUAGE.LangButtonText = "Установить язык"
LANGUAGE.LangRu = "Русский"
LANGUAGE.LangEn = "Английский"

--Group
LANGUAGE.GroupChangeButton = "Сменить название группы"
LANGUAGE.GroupChangeText1 = "Установите название для группы\n(Название писать в кавычках)"
LANGUAGE.GroupChangeText2 = '"НАЗВАНИЕ"'

--Crafting
LANGUAGE.Craft = "СОЗДАТЬ"
LANGUAGE.CraftNeed = "Тебе нужно :"
LANGUAGE.CraftSheetBasic = "Базовое"
LANGUAGE.CraftSheetAdvanced = "Дополнительное"
LANGUAGE.CraftSheetLegendary = "Легендарное"
LANGUAGE.CraftNo = "У вас нет необходимого навыка"
LANGUAGE.CraftNot = "Вы не можете создать этот предмет."
LANGUAGE.CraftNotDouble = "Вы не можете создать более 5 предметов одновременно."
LANGUAGE.CraftCantCarry = "Вы не можете нести этот предмет. Крафт был отменен."
LANGUAGE.CraftCant = "Вы не можете создать этот предмет. Крафт был отменен."

LANGUAGE.GuitarNo = "Вы не можете поднять предмет, когда держите гитару."

--AirDrop
LANGUAGE.AirDropText = "Высадка уже идет, Пожалуйста, подождите."
LANGUAGE.AirDropText2 = "Вы успешно вызвали AirDrop."
LANGUAGE.AirDropText3 = "На карте в ближайшее время появится AirDrop."

--Stats
LANGUAGE.Stats = "Статистика"
LANGUAGE.Stats_Name = "Имя: "

LANGUAGE.Stats_XP = "XP: "
LANGUAGE.Stats_Money = "Деньги: ₽"
LANGUAGE.Stats_MoneyPack = "пачек/пачка"
LANGUAGE.Stats_Credits = "Деньги: $"

LANGUAGE.Stats_HP = "Кровь: "
LANGUAGE.Stats_Hunger = "Голод: "
LANGUAGE.Stats_Thirst = "Жажда: "
LANGUAGE.Stats_Stamina = "Выносливость: "

--Skills
LANGUAGE.Perk_True = "Получен"
LANGUAGE.Perk_False = "Не Получен"
LANGUAGE.Skills_Cost = "Требуется очков улучшения: "
LANGUAGE.Skills_Unlock = "Разблокировать Талант"
LANGUAGE.Skills_CheckPoint = "Доступно очков улучшения: "

--Shop
LANGUAGE.VipShopLable = "[VIP]Магазин"
LANGUAGE.ShopLable = "Магазин"

LANGUAGE.AmmoTable = "Патроны"
LANGUAGE.WeaponTable = "Оружие"
LANGUAGE.SurviveTable = "Выживание"
LANGUAGE.InstrumentsTable = "Инструменты"
LANGUAGE.TrashTable = "Мусор"
LANGUAGE.ModifyTable = "Модификации"
LANGUAGE.PerkTable = "Перки"
LANGUAGE.FoodTable = "Еда"

LANGUAGE.ClipSize = "Патрон: "
LANGUAGE.Cost = "Стоимость"
LANGUAGE.PriceMoney = "Покупка: ₽"
LANGUAGE.PriceMoneyButton = "Купить за ₽"
LANGUAGE.PriceCredits = "Покупка: $"
LANGUAGE.PriceCreditsButton = "Купить за $"
LANGUAGE.SellMoney = "Продажа: ₽"
LANGUAGE.SellMoneyButton = "Продать за ₽"

LANGUAGE.NotEnoughMoney = "Вы не можете себе это позволить."
LANGUAGE.NotWeight = "Не хватает места."
LANGUAGE.NotNPC = "Не получится. Подойди к продавцу и купи/продай."
LANGUAGE.BuyItem = "Ты купил "
LANGUAGE.SellItem = "Ты продал "
LANGUAGE.NotHaveItem = "У тебя нет этого предмета."
LANGUAGE.NotHaveItemBackPack = "Этого предмета нет в рюкзаке."

--Party
LANGUAGE.KickPlayerGroup = "Вы выгнали %s из группы."
LANGUAGE.NotLeaderGroup = "Ты не лидер группы."
LANGUAGE.AlreadyInviteGroup = "Этот игрок уже получал приглашение."
LANGUAGE.AlreadyTargetGroup1 = "Этот игрок уже в группе."
LANGUAGE.InviteYou1 = "%s предложил вам вступить в его группу."
LANGUAGE.AlreadyTargetGroup2 = "%s отправил вам приглашение в группу."
LANGUAGE.InviteYou2 = "Ты отправил игроку %s приглашение в группу."
LANGUAGE.JoinGroupYou = "Вы присоединились к группе игрока %s."
LANGUAGE.JoinGroupTarget = "%s присоединился к группе."
LANGUAGE.GroupNoPlayer = "Ваша группа была расформирована т.к в ней не было людей."
LANGUAGE.LeaderRemoveGroup = "Лидер распустил группу."
LANGUAGE.TargetLeaveGroup = "%s вышел из группы."
LANGUAGE.LeaveGroup = "Ты покинул группу."

--Perk
LANGUAGE.Perk_buy = "Ты разблокировал перк: "
LANGUAGE.Skill_buy = "Ты изучил "
LANGUAGE.Perk_notbuy = "У вас уже есть этот перк."
LANGUAGE.Skill_notbuy1 = "У вас уже есть этот талант."
LANGUAGE.Skill_notbuy2 = "Ты не можешь прокачать этот талант т.к ты не прокачал талант до него."
LANGUAGE.Skill_notbuy3 = "У вас недостаточно очков улучшения чтобы изучить этот талант."

LANGUAGE.Perk1_name = "Крафтинг 1"
LANGUAGE.Perk1_desc = "Позвольте вам создавать базовые предметы"

LANGUAGE.Perk2_name = "Крафтинг 2"
LANGUAGE.Perk2_desc = "Позвольте вам создавать среднего качества предметы"

LANGUAGE.Perk3_name = "Легендарная Лицензия На Крафт"
LANGUAGE.Perk3_desc = ""

LANGUAGE.Perk4_name = "Крафтинг 3"
LANGUAGE.Perk4_desc = "Позвольте вам создавать легендарные предметы"

LANGUAGE.Perk5_name = "EMPTY SKILL (NOT USED)"
LANGUAGE.Perk5_desc = ""

LANGUAGE.Perk6_name = "EMPTY SKILL (NOT USED)"
LANGUAGE.Perk6_desc = ""

LANGUAGE.Perk7_name = "Битва с голодом[1]"
LANGUAGE.Perk7_desc = "Вы все меньше хотите кушать"

LANGUAGE.Perk8_name = "Первая помощь"
LANGUAGE.Perk8_desc = "Все лечащие предметы становятся более эффективными"

LANGUAGE.Perk9_name = "Выживший"
LANGUAGE.Perk9_desc = "От жажды и голода ваше здоровье уменьшается медленнее "

LANGUAGE.Perk10_name = "Битва с голодом[2]"
LANGUAGE.Perk10_desc = "Вы все меньше хотите пить"

LANGUAGE.Perk11_name = "Закалённый в боях"
LANGUAGE.Perk11_desc = "Вы получаете меньше урона.\n(понижает урон от игроков и зомби на 15%)"

LANGUAGE.Perk12_name = "Лицензия на вождение"
LANGUAGE.Perk12_desc = "Позволяет Вам ездить на машине"

LANGUAGE.Perk13_name = "Механик[1]"
LANGUAGE.Perk13_desc = "Позволяет Вам ремонтировать технику"

LANGUAGE.Perk14_name = "Механик[2]"
LANGUAGE.Perk14_desc = "Вы чините технику в два раза эффективнее"

LANGUAGE.Perk15_name = "Ближний бой 1"
LANGUAGE.Perk15_desc = "Увеличивает урон в ближнем бою против игроков"

LANGUAGE.Perk16_name = "Ближний бой 2"
LANGUAGE.Perk16_desc = "Увеличивает урон в ближнем бою против игроков"

LANGUAGE.Perk17_name = "Ближний бой 3"
LANGUAGE.Perk17_desc = "Увеличивает урон в ближнем бою против игроков"

LANGUAGE.Perk18_name = "Устойчивость"
LANGUAGE.Perk18_desc = "Снижает отдачу оружия"

LANGUAGE.Perk19_name = "На мушке"
LANGUAGE.Perk19_desc = "Уменьшает разброс у оружия"

LANGUAGE.Perk20_name = "Профессиональный стрелок"
LANGUAGE.Perk20_desc = "Урон от пуль повышается на 15%"

--ItemList
LANGUAGE.ID1_item = "Деньги($)"
LANGUAGE.ID1_desc = "Премиумные деньги."
LANGUAGE.ID01_usetip = ""
LANGUAGE.ID01_blocktip = ""

LANGUAGE.ID2_item = "Деньги(₽)"
LANGUAGE.ID2_desc = "Без денег ты пропадешь."
LANGUAGE.ID2_usetip = "Недостаточно денег чтобы их сложить."
LANGUAGE.ID2_blocktip = ""

LANGUAGE.ID3_item = "Пачка Денег"
LANGUAGE.ID3_desc = "Без денег ты пропадешь."
LANGUAGE.ID3_usetip = ""
LANGUAGE.ID3_blocktip = ""

LANGUAGE.ID4_item = "Какая-то Снайперская Винтовка"
LANGUAGE.ID4_desc = ""
LANGUAGE.ID4_usetip = ""
LANGUAGE.ID4_blocktip = ""

LANGUAGE.ID5_item = "Какая-то винтовка"
LANGUAGE.ID5_desc = ""
LANGUAGE.ID5_usetip = ""
LANGUAGE.ID5_blocktip = ""

LANGUAGE.ID6_item = "Какая-то Винтовка"
LANGUAGE.ID6_desc = ""
LANGUAGE.ID6_usetip = ""
LANGUAGE.ID6_blocktip = ""

LANGUAGE.ID7_item = "Какой-то ПП"
LANGUAGE.ID7_desc = ""
LANGUAGE.ID7_usetip = ""
LANGUAGE.ID7_blocktip = ""

LANGUAGE.ID8_item = "M21"
LANGUAGE.ID8_desc = "Снайперская винтовка, с самозарядным механизмом."
LANGUAGE.ID8_usetip = ""
LANGUAGE.ID8_blocktip = ""

LANGUAGE.ID9_item = "M24"
LANGUAGE.ID9_desc = "Снайперская винтовка , с болтовым механизмом"
LANGUAGE.ID9_usetip = ""
LANGUAGE.ID9_blocktip = ""

LANGUAGE.ID10_item = "RPK"
LANGUAGE.ID10_desc = "Ручной пулемет Калашникова, с большым уроном."
LANGUAGE.ID10_usetip = ""
LANGUAGE.ID10_blocktip = ""

LANGUAGE.ID11_item = "M4A1"
LANGUAGE.ID11_desc = "Штурмовая винтовка, с балансироваными хар-рак."
LANGUAGE.ID11_usetip = ""
LANGUAGE.ID11_blocktip = ""

LANGUAGE.ID12_item = "AK-74"
LANGUAGE.ID12_desc = "Автомат, с русской надежностью. "
LANGUAGE.ID12_usetip = ""
LANGUAGE.ID12_blocktip = ""

LANGUAGE.ID13_item = "FAMAS"
LANGUAGE.ID13_desc = "Штурмовая винтовка, системы булл-пап"
LANGUAGE.ID13_usetip = ""
LANGUAGE.ID13_blocktip = ""

LANGUAGE.ID14_item = "IMI Galil"
LANGUAGE.ID14_desc = "Автомат, имеющий сходную компоновку с АК."
LANGUAGE.ID14_usetip = ""
LANGUAGE.ID14_blocktip = ""

LANGUAGE.ID15_item = "G36C"
LANGUAGE.ID15_desc = "Автомат, имеющий исключительно малую отдачу "
LANGUAGE.ID15_usetip = ""
LANGUAGE.ID15_blocktip = ""

LANGUAGE.ID16_item = "SG 552"
LANGUAGE.ID16_desc = "Штурмовая винтовка, c большим выбором модификаций."
LANGUAGE.ID16_usetip = ""
LANGUAGE.ID16_blocktip = ""

LANGUAGE.ID17_item = "KS-23"
LANGUAGE.ID17_desc = "Специальный карабин, с высокой поражающей силой."
LANGUAGE.ID17_usetip = ""
LANGUAGE.ID17_blocktip = ""

LANGUAGE.ID18_item = "Remington 870"
LANGUAGE.ID18_desc = "Дробовик, с большим разбросом дроби."
LANGUAGE.ID18_usetip = ""
LANGUAGE.ID18_blocktip = ""

LANGUAGE.ID19_item = "MP5k"
LANGUAGE.ID19_desc = "Пистолет-пулемет, созданный для спец-операций."
LANGUAGE.ID19_usetip = ""
LANGUAGE.ID19_blocktip = ""

LANGUAGE.ID20_item = "IMI Uzi"
LANGUAGE.ID20_desc = "Пистолет-пулемет, с высокой скоростью стрельбы."
LANGUAGE.ID20_usetip = ""
LANGUAGE.ID20_blocktip = ""

LANGUAGE.ID21_item = "PP19(«Бизон»)"
LANGUAGE.ID21_desc = "Пистолет-пулемет, имеющий магазин на 64 патрона."
LANGUAGE.ID21_usetip = ""
LANGUAGE.ID21_blocktip = ""

LANGUAGE.ID22_item = "Desert Eagle"
LANGUAGE.ID22_desc = "Самозарядный пистолет крупного калибра"
LANGUAGE.ID22_usetip = ""
LANGUAGE.ID22_blocktip = ""

LANGUAGE.ID23_item = "M1911"
LANGUAGE.ID23_desc = "Пистолет M1911, с хорошей точностью стрельбы."
LANGUAGE.ID23_usetip = ""
LANGUAGE.ID23_blocktip = ""

LANGUAGE.ID24_item = "Glock-20"
LANGUAGE.ID24_desc = "Пистолет Glock-20, с большой емкостью магазина."
LANGUAGE.ID24_usetip = ""
LANGUAGE.ID24_blocktip = ""

LANGUAGE.ID25_item = "Граната"
LANGUAGE.ID25_desc = ""
LANGUAGE.ID25_usetip = ""
LANGUAGE.ID25_blocktip = ""

LANGUAGE.ID26_item = "Какое-то оружие ближнего боя"
LANGUAGE.ID26_desc = ""
LANGUAGE.ID26_usetip = ""
LANGUAGE.ID26_blocktip = ""

LANGUAGE.ID27_item = "Бита"
LANGUAGE.ID27_desc = "Деревянная бита, с цветом алюминия"
LANGUAGE.ID27_usetip = ""
LANGUAGE.ID27_blocktip = ""

LANGUAGE.ID28_item = "Топор"
LANGUAGE.ID28_desc = "Немного ржавый топор для рубки леса или для рубки чего-то еще/"
LANGUAGE.ID28_usetip = ""
LANGUAGE.ID28_blocktip = ""

LANGUAGE.ID29_item = "Монтировка"
LANGUAGE.ID29_desc = "HL3 CONFIRMED"
LANGUAGE.ID29_usetip = ""
LANGUAGE.ID29_blocktip = ""

LANGUAGE.ID30_item = "Патроны для Снайперской Винтовки"
LANGUAGE.ID30_desc = "Универсальные патроны для всех снайперских винтовок."
LANGUAGE.ID30_usetip = ""
LANGUAGE.ID30_blocktip = ""

LANGUAGE.ID31_item = "Патроны для Винтовки/Автомата"
LANGUAGE.ID31_desc = "Универсальные патроны для всех штурмовых винтовок/автоматов."
LANGUAGE.ID31_usetip = ""
LANGUAGE.ID31_blocktip = ""

LANGUAGE.ID32_item = "Патроны для Пистолета-Пулемета"
LANGUAGE.ID32_desc = "Универсальные патроны для всех пистолетов-пулеметов."
LANGUAGE.ID32_usetip = ""
LANGUAGE.ID32_blocktip = ""

LANGUAGE.ID33_item = "Патроны для Дробовика/Карабина"
LANGUAGE.ID33_desc = "Универсальная дробь для всех видов дробовиков/карабинов."
LANGUAGE.ID33_usetip = ""
LANGUAGE.ID33_blocktip = ""

LANGUAGE.ID34_item = "Патроны для Пистолета"
LANGUAGE.ID34_desc = "Универсальные патроны для всех видов пистолета."
LANGUAGE.ID34_usetip = ""
LANGUAGE.ID34_blocktip = ""

LANGUAGE.ID35_item = "Рюкзак выживателя"
LANGUAGE.ID35_desc = "Специализированный рюкзак выживателя, с доп. карманами."
LANGUAGE.ID35_usetip = ""
LANGUAGE.ID35_blocktip = ""

LANGUAGE.ID36_item = "Спортивная сумка"
LANGUAGE.ID36_desc = "Спортивная сумка из плотной ткани, непременный атрибут спортсменов, пассажиров поездов в 90х"
LANGUAGE.ID36_usetip = ""
LANGUAGE.ID36_blocktip = ""

LANGUAGE.ID37_item = "Туристический рюкзак Пилигримм"
LANGUAGE.ID37_desc = "Большой и вместительный туристический рюкзак яркой расцветки."
LANGUAGE.ID37_usetip = ""
LANGUAGE.ID37_blocktip = ""

LANGUAGE.ID38_item = "Рюкзак Blackjack 50 (Multicam)"
LANGUAGE.ID38_desc = "Рюкзак для дальних и успешных рейдов от фирмы Mystery Ranch"
LANGUAGE.ID38_usetip = ""
LANGUAGE.ID38_blocktip = ""

LANGUAGE.ID39_item = "Форма"
LANGUAGE.ID39_desc = ""
LANGUAGE.ID39_usetip = ""
LANGUAGE.ID39_blocktip = ""

LANGUAGE.ID40_item = "Форма Пилота"
LANGUAGE.ID40_desc = "Форма турецкого пилота"
LANGUAGE.ID40_usetip = "Ты одел Форму Пилота"
LANGUAGE.ID40_blocktip = ""

LANGUAGE.ID41_item = "Химическая Защита"
LANGUAGE.ID41_desc = "Костюм войск РХБЗ"
LANGUAGE.ID41_usetip = "Ты одел Химическую Защиту"
LANGUAGE.ID41_blocktip = ""

LANGUAGE.ID42_item = "Форма Ф.С.Б"
LANGUAGE.ID42_desc = "Броня ФСБ, с самым нужным - стальными пластинам и шлемом"
LANGUAGE.ID42_usetip = "Ты одел форму Ф.С.Б"
LANGUAGE.ID42_blocktip = ""

LANGUAGE.ID43_item = "Маскировочный Халат"
LANGUAGE.ID43_desc = "Военный Мас.Халат, для ведения скрытного боя в лесу."
LANGUAGE.ID43_usetip = "Ты одел Мас.Халат"
LANGUAGE.ID43_blocktip = ""

LANGUAGE.ID44_item = "Форма Российского солдата"
LANGUAGE.ID44_desc = "Экипировка Ратник-3"
LANGUAGE.ID44_usetip = "Ты одел Форму Российского солдата"
LANGUAGE.ID44_blocktip = ""

LANGUAGE.ID45_item = "Армейская униформа"
LANGUAGE.ID45_desc = "Армейская униформа с удобными подсумками и пр."
LANGUAGE.ID45_usetip = "Ты одел униформу"
LANGUAGE.ID45_blocktip = ""

LANGUAGE.ID46_item = "Модификация"
LANGUAGE.ID46_desc = ""
LANGUAGE.ID46_usetip = ""
LANGUAGE.ID46_blocktip = ""

LANGUAGE.ID47_item = "Модификациия ACOG"
LANGUAGE.ID47_desc = "Прицел ACOG, для ведения дальнего боя."
LANGUAGE.ID47_usetip = ""
LANGUAGE.ID47_blocktip = ""

LANGUAGE.ID48_item = "Модификациия C79"
LANGUAGE.ID48_desc = "Прицел C79, для ведения дальнего боя."
LANGUAGE.ID48_usetip = ""
LANGUAGE.ID48_blocktip = ""

LANGUAGE.ID49_item = "Модификациия CompM4"
LANGUAGE.ID49_desc = "Прицел CompM4, для ведения боя на средней дистанции."
LANGUAGE.ID49_usetip = ""
LANGUAGE.ID49_blocktip = ""

LANGUAGE.ID50_item = "Модификациия EoTech 553"
LANGUAGE.ID50_desc = "Колиматорный прицел EoTech 553, для ведения ближнего боя."
LANGUAGE.ID50_usetip = ""
LANGUAGE.ID50_blocktip = ""

LANGUAGE.ID51_item = "Модификациия Foregrip"
LANGUAGE.ID51_desc = ""
LANGUAGE.ID51_usetip = ""
LANGUAGE.ID51_blocktip = ""

LANGUAGE.ID52_item = "Модификациия Harris Bipod"
LANGUAGE.ID52_desc = "Сошки Harris Bipod, для снайперских винтовок."
LANGUAGE.ID52_usetip = ""
LANGUAGE.ID52_blocktip = ""

LANGUAGE.ID53_item = "Модификациия Leupold MK4"
LANGUAGE.ID53_desc = ""
LANGUAGE.ID53_usetip = ""
LANGUAGE.ID53_blocktip = ""

LANGUAGE.ID54_item = "Модификациия M21 20RND Mag"
LANGUAGE.ID54_desc = "Увеличительный магазин на Снайперскую Винтовку M21."
LANGUAGE.ID54_usetip = ""
LANGUAGE.ID54_blocktip = ""

LANGUAGE.ID55_item = "Модификациия MP5K 30RND Mag"
LANGUAGE.ID55_desc = "Увеличительный магазин на ПП MP5K"
LANGUAGE.ID55_usetip = ""
LANGUAGE.ID55_blocktip = ""

LANGUAGE.ID56_item = "Модификациия PSO-1"
LANGUAGE.ID56_desc = "Снайперский прицел PSO-1, для ведения боя на дальней дистанции."
LANGUAGE.ID56_usetip = ""
LANGUAGE.ID56_blocktip = ""

LANGUAGE.ID57_item = "Модификациия SG55X 30RND Mag"
LANGUAGE.ID57_desc = "Увеличительный магазин на SG502."
LANGUAGE.ID57_usetip = ""
LANGUAGE.ID57_blocktip = ""

LANGUAGE.ID58_item = "Модификациия Suppressor"
LANGUAGE.ID58_desc = "Универсальный ПБС."
LANGUAGE.ID58_usetip = ""
LANGUAGE.ID58_blocktip = ""

LANGUAGE.ID59_item = "Модификациия Tritium Sights"
LANGUAGE.ID59_desc = "Тритиевый прицел, для лучшего наведения на цель."
LANGUAGE.ID59_usetip = ""
LANGUAGE.ID59_blocktip = ""

LANGUAGE.ID60_item = "Модификациия UZI Wooden Stock"
LANGUAGE.ID60_desc = "Деревянный приклад для IMI UZI, снижающий отдачу."
LANGUAGE.ID60_usetip = ""
LANGUAGE.ID60_blocktip = ""

LANGUAGE.ID61_item = "Индивидуальный рацион питания"
LANGUAGE.ID61_desc = "Сколько лет прошло... Это вообще съедобно?"
LANGUAGE.ID61_usetip = "Я съел весь Индивидуальный рацион питания"
LANGUAGE.ID61_blocktip = "Идет использование ИРП"

LANGUAGE.ID62_item = "Морфий"
LANGUAGE.ID62_desc = "Болеутоляющее, отлично глушащий боль."
LANGUAGE.ID62_usetip = "Я чуствую себя намного лучше"
LANGUAGE.ID62_blocktip = "Идет использование морфия"

LANGUAGE.ID63_item = "Болеутоляющее"
LANGUAGE.ID63_desc = "Маленькая бутылка болеутоляющего."
LANGUAGE.ID63_usetip = "Я использовал болеутоляющее"
LANGUAGE.ID63_blocktip = "Идет использование болеутоляющего"

LANGUAGE.ID64_item = "Автомобильная аптечка"
LANGUAGE.ID64_desc = "Автомобильная аптечка, со всем необходимым."
LANGUAGE.ID64_usetip = "Я использовал Автомобильную аптечку"
LANGUAGE.ID64_blocktip = "Идет использование автомобильной аптечки"

LANGUAGE.ID65_item = "Бинт"
LANGUAGE.ID65_desc = "Небольшая коробочка с бинтами внутри."
LANGUAGE.ID65_usetip = "Я использовал бинт."
LANGUAGE.ID65_blocktip = "Идет использование бинт"

LANGUAGE.ID66_item = "Фонарик"
LANGUAGE.ID66_desc = "Нажми на [F] чтобы включить фонарик."
LANGUAGE.ID66_usetip = ""
LANGUAGE.ID66_blocktip = ""

LANGUAGE.ID67_item = "Противогаз ГП-5"
LANGUAGE.ID67_desc = "Гражданский противогаз модели 5"
LANGUAGE.ID67_usetip = ""
LANGUAGE.ID67_blocktip = ""

LANGUAGE.ID68_item = "ПНВ"
LANGUAGE.ID68_desc = "Прибор Ночного Виденья. Очень редкая вещь."
LANGUAGE.ID68_usetip = ""
LANGUAGE.ID68_blocktip = ""

LANGUAGE.ID69_item = "Канистра"
LANGUAGE.ID69_desc = "Внутри достаточное количество топлива для заправки авто."
LANGUAGE.ID69_usetip = ""
LANGUAGE.ID69_blocktip = ""

LANGUAGE.ID70_item = "Гаечный ключ"
LANGUAGE.ID70_desc = "Нужен для ремонта автомобиля."
LANGUAGE.ID70_usetip = ""
LANGUAGE.ID70_blocktip = ""

LANGUAGE.ID71_item = ""
LANGUAGE.ID71_desc = ""
LANGUAGE.ID71_usetip = ""
LANGUAGE.ID71_blocktip = ""

LANGUAGE.ID72_item = ""
LANGUAGE.ID72_desc = ""
LANGUAGE.ID72_usetip = ""
LANGUAGE.ID72_blocktip = ""

LANGUAGE.ID73_item = "Газировка Mountain Dew"
LANGUAGE.ID73_desc = "Насыщена электролитами."
LANGUAGE.ID73_usetip = "Я выпил Mountain Dew"
LANGUAGE.ID73_blocktip = "Идет использование газировки Mountain Dew"

LANGUAGE.ID74_item = "Газировка Pepsi"
LANGUAGE.ID74_desc = "Вкус знакомый с детства."
LANGUAGE.ID74_usetip = "Я выпил Газировку"
LANGUAGE.ID74_blocktip = "Идет использование газировки Pepsi"

LANGUAGE.ID75_item = "Газировка Coca-Cola"
LANGUAGE.ID75_desc = "Классика среди газ. напитков."
LANGUAGE.ID75_usetip = "Я выпил Газировку"
LANGUAGE.ID75_blocktip = "Идет использование газировки Coca-Cola"

LANGUAGE.ID76_item = "Энергетик Monster Energy"
LANGUAGE.ID76_desc = "Углеводы, Таурин, Кофеин..."
LANGUAGE.ID76_usetip = "Я выпил энергетик Monster Energy"
LANGUAGE.ID76_blocktip = "Идет использование энергетика Monster Energy"

LANGUAGE.ID77_item = "Пиво Pibwasser"
LANGUAGE.ID77_desc = "Немецкое забористое пиво."
LANGUAGE.ID77_usetip = "Я выпил пиво Pibwasser"
LANGUAGE.ID77_blocktip = "Идет использование пиво Pibwasser"

LANGUAGE.ID78_item = "Пиво baff"
LANGUAGE.ID78_desc = "Легкое пиво, из города Спрингфилд."
LANGUAGE.ID78_usetip = "Я выпил пиво baff"
LANGUAGE.ID78_blocktip = "Идет использование пиво baff"

LANGUAGE.ID79_item = "Апельсиновый сок"
LANGUAGE.ID79_desc = "Сок с мякотью."
LANGUAGE.ID79_usetip = "Я выпил апельсиновый сок"
LANGUAGE.ID79_blocktip = "Идет использование Апельсинового сока"

LANGUAGE.ID80_item = "Молоко"
LANGUAGE.ID80_desc = "Все еще не прокисшее коровье молоко"
LANGUAGE.ID80_usetip = "Я выпил молоко"
LANGUAGE.ID80_blocktip = "Идет использование молока"

LANGUAGE.ID81_item = "Бутылка с водой"
LANGUAGE.ID81_desc = "Бутылка с чистой водой."
LANGUAGE.ID81_usetip = "Я выпил чистую воду"
LANGUAGE.ID81_blocktip = "Идет использование бутылки с водой"

LANGUAGE.ID82_item = "Бутылка с испорченой водой"
LANGUAGE.ID82_desc = "Такую воду я пить не советовал бы."
LANGUAGE.ID82_usetip = "Я выпил испорченную воду"
LANGUAGE.ID82_blocktip = "Идет использование бутылки с испорченой водой"

LANGUAGE.ID83_item = "Банка с бобами"
LANGUAGE.ID83_desc = "Вполне себе нормальная еда."
LANGUAGE.ID83_usetip = "Я съел банку с бобами"
LANGUAGE.ID83_blocktip = "Идет использование банки с бобами"

LANGUAGE.ID84_item = "Ананас"
LANGUAGE.ID84_desc = "Свежий ананас, с сочной мякотью."
LANGUAGE.ID84_usetip = "Я съел ананас"
LANGUAGE.ID84_blocktip = "Идет использование ананаса"

LANGUAGE.ID85_item = "Бананы"
LANGUAGE.ID85_desc = "Спелые бананы с Африки."
LANGUAGE.ID85_usetip = "Я съел бананы"
LANGUAGE.ID85_blocktip = "Идет использование бананов"

LANGUAGE.ID86_item = "Куриная ножка"
LANGUAGE.ID86_desc = "Зажаренная куриная ножка."
LANGUAGE.ID86_usetip = "Я съел куриную ножку"
LANGUAGE.ID86_blocktip = "Идет использование куриной ножки"

LANGUAGE.ID87_item = "Пирог"
LANGUAGE.ID87_desc = "Яблочный пирог."
LANGUAGE.ID87_usetip = "Я съел пирог"
LANGUAGE.ID87_blocktip = "Идет использование пирога"

LANGUAGE.ID88_item = "Чипсы Lays"
LANGUAGE.ID88_desc = "Классические чипсы Lays."
LANGUAGE.ID88_usetip = "Я съел чипсы Lays"
LANGUAGE.ID88_blocktip = "Идет использование чипсов Lays"

LANGUAGE.ID89_item = "Чипсы Doritos"
LANGUAGE.ID89_desc = "Чипсы Doritos со вкусом weed-а"
LANGUAGE.ID89_usetip = "Я съел чипсы Doritos"
LANGUAGE.ID89_blocktip = "Идет использование чипсов Doritos"

LANGUAGE.ID90_item = "Сыр"
LANGUAGE.ID90_desc = "Кружок сыра."
LANGUAGE.ID90_usetip = "Я съел сыр."
LANGUAGE.ID90_blocktip = "Идет использование сыра"

LANGUAGE.ID91_item = "Жареный омар"
LANGUAGE.ID91_desc = "Деликатес для гурманов."
LANGUAGE.ID91_usetip = "Я съел жареного омара"
LANGUAGE.ID91_blocktip = "Идет использование жареного омара"

LANGUAGE.ID92_item = "Сырой омар"
LANGUAGE.ID92_desc = "Если его пожарить, то получится деликатес. Но как?"
LANGUAGE.ID92_usetip = "Я съел сырого омара"
LANGUAGE.ID92_blocktip = "Идет использование сырого омара"

LANGUAGE.ID93_item = "Жареная курятина"
LANGUAGE.ID93_desc = "Ничего лишнего, только соль и перец."
LANGUAGE.ID93_usetip = "Я съел жареную курятину."
LANGUAGE.ID93_blocktip = "Идет использование жаренной курятины"

LANGUAGE.ID94_item = "Стейк"
LANGUAGE.ID94_desc = "Medium-Rare всегда вкусно."
LANGUAGE.ID94_usetip = "Я съел стейк"
LANGUAGE.ID94_blocktip = "Идет использование стейка"

LANGUAGE.ID95_item = "Жареный бекон"
LANGUAGE.ID95_desc = "Можно быстро подкрепится сьев пару штучек."
LANGUAGE.ID95_usetip = "Я съел жаренный бекон"
LANGUAGE.ID95_blocktip = "Идет использование жареного бекона"

LANGUAGE.ID96_item = "Сырая курятина"
LANGUAGE.ID96_desc = "Общипанная, осталось только приготовить."
LANGUAGE.ID96_usetip = "Я съел сырую курятину."
LANGUAGE.ID96_blocktip = "Идет использование сырой курятины"

LANGUAGE.ID97_item = "Сырой стейк"
LANGUAGE.ID97_desc = "Если приготовить, то будет очень вкусно."
LANGUAGE.ID97_usetip = "Я съел сырой стейк"
LANGUAGE.ID97_blocktip = "Идет использование сырого стейка"

LANGUAGE.ID98_item = "Сырой бекон"
LANGUAGE.ID98_desc = "Вкусно конечно, но лучше поджарить."
LANGUAGE.ID98_usetip = "Я съел сырой бекон."
LANGUAGE.ID98_blocktip = "Идет использование сырого бекона"

LANGUAGE.ID99_item = "ЯЩИК ПАНДОРЫ(РАНДОМНАЯ ВЕЩЬ)"
LANGUAGE.ID99_desc = "Никто не знает что там лежит! Люди лишь только догадываются."
LANGUAGE.ID99_usetip = "У вас недостаточно места чтобы открыть ящик."
LANGUAGE.ID99_blocktip = ""

LANGUAGE.ID100_item = "Кукла"
LANGUAGE.ID100_desc = "А ведь в нее когда-то играли дети"
LANGUAGE.ID100_usetip = ""
LANGUAGE.ID100_blocktip = ""

LANGUAGE.ID101_item = "Кейс"
LANGUAGE.ID101_desc = "Здесь лежат какие-то документы."
LANGUAGE.ID101_usetip = ""
LANGUAGE.ID101_blocktip = ""

LANGUAGE.ID102_item = "Конус"
LANGUAGE.ID102_desc = "Сейчас от него толку ноль."
LANGUAGE.ID102_usetip = ""
LANGUAGE.ID102_blocktip = ""

LANGUAGE.ID103_item = "Бюст Брина"
LANGUAGE.ID103_desc = "Славу богу что Альянс свергнули..."
LANGUAGE.ID103_usetip = ""
LANGUAGE.ID103_blocktip = ""

LANGUAGE.ID104_item = "Краска"
LANGUAGE.ID104_desc = "Нам действительно что-то нужно покрасить?"
LANGUAGE.ID104_usetip = ""
LANGUAGE.ID104_blocktip = ""

LANGUAGE.ID105_item = "Порох"
LANGUAGE.ID105_desc = "Используется для изготовления любого типа боеприпасов"
LANGUAGE.ID105_usetip = ""
LANGUAGE.ID105_blocktip = ""

LANGUAGE.ID106_item = "Металл"
LANGUAGE.ID106_desc = "Это нужно для крафта чего-либо."
LANGUAGE.ID106_usetip = ""
LANGUAGE.ID106_blocktip = ""

LANGUAGE.ID107_item = "Кусок древесины"
LANGUAGE.ID107_desc = "Это нужно для крафта чего-либо."
LANGUAGE.ID107_usetip = ""
LANGUAGE.ID107_blocktip = ""

LANGUAGE.ID108_item = "Контейнер с Тканью"
LANGUAGE.ID108_desc = "Нужно для крафта вещей :)"
LANGUAGE.ID108_usetip = "Я скрафтил бинт."
LANGUAGE.ID108_blocktip = "Идет использование Контейнера с Тканью"

LANGUAGE.ID109_item = "Пустая жестяная банка"
LANGUAGE.ID109_desc = "Когда-то в ней была еда."
LANGUAGE.ID109_usetip = ""
LANGUAGE.ID109_blocktip = ""

LANGUAGE.ID110_item = "МОЩЬ ГОР"
LANGUAGE.ID110_desc = "Увеличивает вашу выносливость"
LANGUAGE.ID110_usetip = ""
LANGUAGE.ID110_blocktip = ""

LANGUAGE.ID111_item = "Системный блок"
LANGUAGE.ID111_desc = "Простой системный блок,со всеми приблудами внутри"
LANGUAGE.ID111_usetip = ""
LANGUAGE.ID111_blocktip = ""

LANGUAGE.ID112_item = "Череп"
LANGUAGE.ID112_desc = "'О, бедный Йорик! Я знал его, Горацио' ©Уильям Шекспир «Гамлет»"
LANGUAGE.ID112_usetip = ""
LANGUAGE.ID112_blocktip = ""

LANGUAGE.ID113_item = "Глобус"
LANGUAGE.ID113_desc = "Глобус как глобус. Хотя стоп,а где здесь мы?"
LANGUAGE.ID113_usetip = ""
LANGUAGE.ID113_blocktip = ""

LANGUAGE.ID114_item = "Чайник"
LANGUAGE.ID114_desc = "На нем можно кипятить воду,но как??"
LANGUAGE.ID114_usetip = ""
LANGUAGE.ID114_blocktip = ""

LANGUAGE.ID115_item = "Книга"
LANGUAGE.ID115_desc = "'YouTube. Путь к успеху.' Бред,а не книга."
LANGUAGE.ID115_usetip = ""
LANGUAGE.ID115_blocktip = ""

LANGUAGE.ID116_item = "Старая газета"
LANGUAGE.ID116_desc = "Старая газета с City-17. Ничего интересного."
LANGUAGE.ID116_usetip = ""
LANGUAGE.ID116_blocktip = ""

LANGUAGE.ID117_item = "Гитара"
LANGUAGE.ID117_desc = "Струнный музыкальный инструмент, который считается \nсамым распространенным в мире"
LANGUAGE.ID117_usetip = ""
LANGUAGE.ID117_blocktip = ""

LANGUAGE.ID118_item = "Рация"
LANGUAGE.ID118_desc = ""
LANGUAGE.ID118_usetip = ""
LANGUAGE.ID118_blocktip = ""

LANGUAGE.ID119_item = "AirDrop Граната"
LANGUAGE.ID119_desc = "Вам нужны припасы? Бросьте это, чтобы запросить воздушную высадку, но будьте осторожны. Другие игроки смогут увидеть и украсть его."
LANGUAGE.ID119_usetip = ""
LANGUAGE.ID119_blocktip = ""